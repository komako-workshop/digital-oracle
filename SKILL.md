...
- Eastmoney throttles per source IP. The provider falls back to the delay host automatically, which covers quotes / fund flow / sectors but **not** daily K-line — if `get_history` raises "every Eastmoney host failed", you are throttled: use `YahooPriceProvider` with `.SS` / `.SZ` for the price series instead, and keep Eastmoney for the flow data it uniquely provides
